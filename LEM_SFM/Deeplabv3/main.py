...
def infer_1_img():
    ...
    img_path = '/media/pan/089614D79614C6DA/temp/Linux/code/Deeplabv3-master/1305031790.645155.png'
    img = Image.open(img_path)
    img = np.array(img)
    from cv2 import resize
    img=resize(img, None, fx=2, fy=2)
    print(img.shape)
    img_tensor = torch.from_numpy(img).float()
    img_tensor = img_tensor.permute(2,0, 1).unsqueeze(0)  # 添加批量维度和通道维度 -> (1, 1, H, W)
    ...
    checkpoint_path = '/media/pan/089614D79614C6DA/temp/Linux/code/Deeplabv3-master/best_deeplabv3plus_mobilenet_voc_os16.pth'
    model.load_state_dict( torch.load( checkpoint_path, weights_only=False)['model_state']  )
    ...
    # 用plt.imshow可视化输出
    # pred = output.argmax(dim=1).squeeze().cpu().numpy()  # 获取预测结果
    for i in range(21):
        pred = output[0,i,:,:].cpu().numpy()
        print(pred.shape)
        pred = np.array(pred, dtype=np.uint8)
        pred[pred == 255] = 19  #loader.dataset.decode_target(pred).astype(np.uint8)
        plt.imsave('output_%d.png' % i, pred, cmap='gray')
