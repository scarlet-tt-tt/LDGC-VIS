...
class TensorBoardLogger(object):
    ...
    def write_to_terminal(self, display_dict, epoch=None, batch_iter=None, epoch_len=None, batch_time=None, is_train = True):
    ...
    def write_to_terminal_val(self, display_dict,epoch, is_train = True):
        """ Write the save states (display_dict) and training information to terminal for display
        """
        if is_train:
            prefix = 'Train'
            print('EEROR TERMINAL')
        else:
            prefix = 'Test'+ ':\tEpoch %d'%(epoch+1)
