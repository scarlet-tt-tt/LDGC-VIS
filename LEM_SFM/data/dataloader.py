...
def load_data(dataset_name,dataroot, keyframes = None, load_type = 'train',
              trajectory='',load_numpy = False):
    ...
    if dataset_name == 'TUM_RGBD':
        ...
        # np_loader = TUM(dataroot, load_type, keyframes, data_transform, select_trajectory)
        from LEM_SFM.data.TUM import TUM_Dataset
        # print('dataroot:',dataroot)
        np_loader = TUM_Dataset(dataroot=dataroot, 
                                category=load_type, 
                                keyframes=keyframes, 
...
# train_set = load_data_mix(dataset_name, input_shape, train)

data_dir = [
    '/home/data/TUM',
    '/home/data/CambridgeLandmarks',
    '/home/data/7Scenes',
]

def load_data_mix(dataset_name_list,dataroot, input_shape,keyframes = None, load_type = 'train',
              trajectory='',load_numpy = False):
    """ Use two frame camera pose data loader
    """
    if trajectory == '':
    ...
    if 'TUM_RGBD' in dataset_name_list:
        from data.TUM import TUM_Dataset
        np_loader = TUM_Dataset(dataroot=dataroot, 
                                category=load_type, 
                                keyframes=keyframes, 
                                data_transform=data_transform,
