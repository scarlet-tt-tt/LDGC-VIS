...
class TUM(data.Dataset): 
    ...
    def __init__(self, dataroot, category='train', 
        ...
        if category == 'test':
            self.__load_test(dataroot, select_traj)
        else: # train and validation
            self.__load_train_val(dataroot, category)
    ...
    def __load_train_val(self, dataroot, category):
        ...
        for ks, scene in tum_data.items():
            for seq_name in scene['seq']: 
                ...
                sync_traj_file = osp.join(dataroot, seq_path, 'sync_trajectory.pkl') 
                if not osp.isfile(sync_traj_file):
                    ...
                    write_sync_trajectory(dataroot, ks, seq_name)
    ...
    def __load_test(self, dataroot, select_traj=None):
        """ Note: 
    ...
    def __load_rgb_tensor(self, path):
        ...
        image = resize(image, None, fx=self.fx_s, fy=self.fy_s, interpolation=INTER_NEAREST)
        return image
    ...
    def __load_depth_tensor(self, path):
        ...
        """加载深度：
        深度图像按5000倍缩放，
        即深度图像中的像素值5000对应于距离相机1米的距离、10000到2米的距离等。像素值为0表示缺失值/无数据。
        """
        # print('=====================================')
