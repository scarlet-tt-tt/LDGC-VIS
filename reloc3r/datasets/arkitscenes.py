...
DATA_ROOT = os.environ.get("ARKITSCENES_ROOT", "/home/dust3r/arkitscenes_processed")
...
class ARKitScenes(BaseStereoViewDataset):
    ...
    def _load_data(self, split):
        with np.load(osp.join(self.ROOT, split, 'new.npz')) as data:
            ...
            fr = 1
            if fr ==4:
                self.pairs =  data['pairs_4']
            elif fr ==2:
                self.pairs = data['pairs_2']
            elif fr ==1:
                self.pairs = data['pairs_1']
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        for view_idx in [image_idx1, image_idx2]:
            ...
            scene_dir = osp.join(self.ROOT, self.split, self.scenes[view_idx])
            ...
            views.append(dict(
                ...
                dataset='arkitscenes',
                label=self.scenes[view_idx] + '_' + basename,
...
#     def __len__(self):
#         return len(self.images)-self.fr
#         # return len(self.pairs)
...
#             rgb_image = imread_cv2(osp.join(scene_dir, 'vga_wide', basename.replace('.png', '.jpg')))
#             # img_path = osp.join(scene_dir, 'vga_wide', basename.replace('.png', '.jpg'))
